...
test("submission wrapper preserves route configuration and original outcome", async () => {
  const config = {
    attemptId: "0198ef00-0000-7000-8000-000000000601",
    route: { routeId: "rpc-primary" },
    attemptSequence: 1,
    encoding: "base64",
    skipPreflight: false,
  };
  ...
  const failed = await submitWithRoute(config, async () => {
    throw error;
  });
...
test("signed-byte fingerprint uses exact bytes and wipes its transient copy", () => {
  ...
  let observed;
  const result = fingerprintSignedBytesInMemory(
    "0198ef00-0000-7000-8000-000000000900",
    input,
    (bytes) => {
      observed = Array.from(bytes);
      return new Uint8Array(32).fill(bytes[0]);
    },
  );
...
test("signing measurement records monotonic delay without signer access", async () => {
  ...
  const success = await measureSigning(
    async () => "signed-value",
    () => ticks.shift(),
  );
  ...
  const failure = await measureSigning(
    async () => {
      throw error;
    },
    () => failedTicks.shift(),
...
test("simulation capture preserves compute units and classifies execution errors", () => {
  assert.deepEqual(
    normalizeSimulationResult({ err: null, unitsConsumed: 9007199254740993n, logs: ["ok"] }),
    { rpcResult: "succeeded", unitsConsumed: "9007199254740993", logsPresent: true },
  );
  assert.equal(
    normalizeSimulationResult({ err: { InstructionError: [0, "Custom"] } }).rpcResult,
    "execution_error",
  );
  assert.equal(
    normalizeSimulationResult({ blockhashNotFound: true }).rpcResult,
    "blockhash_not_found",
  );
...
test("blockhash capture normalizes an exact validity height", async () => {
  const client = {
    getLatestBlockhash: async () => ({ blockhash: "  hash-1 ", lastValidBlockHeight: 123n }),
    simulate: async () => ({}),
    sign: async () => ({}),
    submit: async () => "sig",
    confirm: async () => ({}),
  };
  assert.deepEqual(await captureLatestBlockhash(client), {
    blockhash: "hash-1",
...
test("adapter boundary preserves customer result and error identity", async () => {
  ...
  await assert.rejects(
    preserveCustomerOperation(async () => {
      throw error;
    }),
