...
const event = {
  schema_version: "1.0", event_id: "0198ef00-0000-7000-8000-00000000c101",
  event_type: "solana.trace.created", occurred_at: "2026-09-13T20:20:00.001Z",
  ...
  privacy_mode: "standard", privacy_policy_version: "1.0", redaction_version: "1.0",
...
test("SDK posts buffered events to the collector and clears a successful batch", async () => {
  ...
  const sdk = new LandfallSdk({
    ...
    fetch: async (url, init) => { requests.push({ url, init }); return { status: 202 }; },
  });
...
test("failed delivery restores the exact batch for a later retry", async () => {
  ...
  const sdk = new LandfallSdk({
    collectorUrl: "http://localhost:8080",
    fetch: async (_url, init) => { calls += 1; return { status: calls === 1 ? 400 : 202, body: init.body }; },
    retry: { maxAttempts: 1 },
  });
  sdk.capture(event);
