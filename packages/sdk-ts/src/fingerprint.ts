...
export function fingerprintSignedBytes(
  keyId: UuidV7,
  serializedSignedBytes: Uint8Array,
  hmac: HmacBytes,
): SignedBytesFingerprint {
  if (serializedSignedBytes.byteLength === 0)
    throw new Error("serialized signed transaction bytes must not be empty");
