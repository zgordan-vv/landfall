...
const required = ["LANDFALL_POLICY_URL", "LANDFALL_X402_TOKEN", "LANDFALL_X402_POLICY_ID", "LANDFALL_X402_AGENT_ID", "X402_RESOURCE_URL", "X402_SIGNER_MODULE"];

if (process.argv.includes("--help")) {
  process.stdout.write(`Runs one real x402 testnet payment. Required variables:\n${required.map((name) => `  ${name}`).join("\n")}\n`);
  process.exit(0);
}
...
const requirement = selectRequirement(challenge.accepts, process.env.X402_NETWORK, process.env.X402_ASSET);
const authorization = preAuthorizeX402Requirement(challenge, requirement, policyId, agentId, idempotencyKey);
...
const outcome = await executeAuthorizedX402Payment(authorization, requirement, authorizer, signer, resourceClient, settlementRecorder);

process.stdout.write(`${JSON.stringify({ kind: outcome.kind, audit_id: outcome.authorization.auditId, decision: outcome.authorization.decision, reason_code: outcome.authorization.reasonCode })}\n`);

function selectRequirement(requirements, network, asset) {
  const selected = requirements.find((candidate) => (network === undefined || candidate.network === network) && (asset === undefined || candidate.asset === asset));
  if (selected === undefined) throw new Error("merchant offered no requirement matching X402_NETWORK/X402_ASSET");
  return selected;
}
...
async function loadSigner(modulePath, context) {
  ...
  if (signer === null || typeof signer !== "object" || typeof signer.createPaymentSignature !== "function") {
